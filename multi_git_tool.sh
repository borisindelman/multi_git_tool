...
function usage {    
    ...
    echo
    echo "Usage: meeseeks.sh [-p] <path/to/repo> [-c/-s] [-e] [repo_list] <command1> <command2> ..."
    echo "* -p: provide path to repo, else Will use GITREPOPATH variable"
    echo "* -e: list of repo names to exclude"
    echo "* -s: status mode"
    echo "* -c: commands mode"
    echo
...
function status_mode {
    ...
    # table header
    echo
    echo -e "\033[1;34m\t| Repo Name$TOTAL_DIR_ENTRY_SPACES| Branch$TOTAL_BRANCH_ENTRY_SPACES| Status$TOTAL_STATUS_ENTRY_SPACES| Staged$TOTAL_STAGED_ENTRY_SPACES| Not Staged$TOTAL_NOT_STAGED_ENTRY_SPACES| Last Commit\033[0;0m"
    ...
    for DIR in $DIRS ; do            
        ...
        echo -e "$STATUS_COLOR($(printf "%02d" $GIT_DIR_NUM)/$TOTAL_GIT_DIRS)\t| $DIR_NAME$DIR_SPACES|$BRANCH$BRANCH_SPACES|$UP_TO_DATE$STATUS_SPACES| $STAGED_STATUS$STAGED_SPACES| $NOT_STAGED_STATUS$NOT_STAGED_SPACES| $LAST_COMMIT_DATE\e[49;39m"                      
    done
}
...
while [[ $# -gt 0 ]] ; do
    ...
    case $key in
        ...
        -c|--command)
            if [ "$MODE" != "" ]; then
                echo "Error. Conflicting command."
                exit 1
            fi
            MODE='command'
            shift # past argument
